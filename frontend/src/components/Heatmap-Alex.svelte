      ...
      try {
        ...
        let tmp: Item[] = rows
          .map((r) => {
            ...
            // Use the "power" field from API (case-insensitive)
            const powerRaw = pick(r, ['power']);
            const parameter = pick(r, ['parameter', 'tf']);
            const powerNum = clampPower(Number(powerRaw));
            return { bot, code, power: powerNum, value: powerNum, parameter: parameter != null ? String(parameter) : null };
          })
          .filter((r) => r.bot === botName && r.code);
  
        if (parameterFilter !== null) {
          tmp = tmp.filter((it) => String(it.parameter ?? '').toLowerCase() === String(parameterFilter).toLowerCase());
        }
  
        // Sort by power descending (same principle as before)
        tmp.sort((a, b) => b.power - a.power);
  
        if (topN && topN > 0) tmp = tmp.slice(0, topN);
  
        items = tmp;
        averagePower = items.length ? items.reduce((s, it) => s + it.power, 0) / items.length : 0;
      } catch (e: any) {
    ...
    <div class="crypto-box">
      ...
      >
        {#each items as it (it.code)}
          <div class="crypto-card" style="background-color: {getColor(it.power)};">
            <img
              class="icon"
            ...
            <p class="crypto-percent">{it.power.toLocaleString(undefined, { maximumFractionDigits: 1 })}%</p>
          </div>
        {/each}
      </div>
      ...
      <div class="progress-bar-container">
        ...
        <div class="progress-marker" style="left: {averagePower}%;"><span class="progress-label">{averagePower.toFixed(1)}%</span></div>
      </div>
    </div>
